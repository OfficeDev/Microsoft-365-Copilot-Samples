    ...
    for entity in entities:
        ...
        if p not in totals:
            totals[p] = {
                ...
                "totalRevenue": float(entity.get("Quantity", 0))
                * float(entity.get("UnitPrice", 0))
                * (1 - float(entity.get("Discount", 0))),
                "totalDiscount": float(entity.get("Quantity", 0))
                * float(entity.get("UnitPrice", 0))
                * float(entity.get("Discount", 0)),
            }
        else:
            totals[p]["totalQuantity"] += float(entity.get("Quantity", 0))
            totals[p]["totalRevenue"] += (
                float(entity.get("Quantity", 0))
                * float(entity.get("UnitPrice", 0))
                * (1 - float(entity.get("Discount", 0)))
            )
            totals[p]["totalDiscount"] += (
                float(entity.get("Quantity", 0))
                * float(entity.get("UnitPrice", 0))
                * float(entity.get("Discount", 0))
            )
...
def get_product_ex_for_entity(entity: dict) -> ProductEx:
    p: ProductEx = {
        ...
        "CategoryName": categories.get(str(entity.get("CategoryID")), {}).get(
            "CategoryName", ""
        ),
        "SupplierName": suppliers.get(str(entity.get("SupplierID")), {}).get(
            "CompanyName", ""
        ),
        "SupplierCity": suppliers.get(str(entity.get("SupplierID")), {}).get(
            "City", ""
        ),
        "InventoryStatus": "",
        "InventoryValue": 0,
    ...
    p["AverageDiscount"] = round(
        p["Revenue"]
        / order_totals.get(p["ProductID"], {}).get(
            "totalDiscount", 1
...
async def get_product_ex(product_id: int) -> ProductEx:
    ...
    entity = table_client.get_entity(
        partition_key=Settings.TABLE_NAME["PRODUCT"],
...
async def update_product(updated_product: ProductEx) -> None:
    ...
    try:
        product = table_client.get_entity(
            partition_key=Settings.TABLE_NAME["PRODUCT"],
            row_key=str(updated_product["ProductID"]),
        )
        temp_product = product
        temp_product["ProductName"] = updated_product["ProductName"]
        temp_product["CategoryID"] = updated_product["CategoryID"]
        temp_product["SupplierID"] = updated_product["SupplierID"]
        temp_product["QuantityPerUnit"] = updated_product["QuantityPerUnit"]
        temp_product["UnitPrice"] = updated_product["UnitPrice"]
        temp_product["UnitsInStock"] = updated_product["UnitsInStock"]
        temp_product["UnitsOnOrder"] = updated_product["UnitsOnOrder"]
        temp_product["ReorderLevel"] = updated_product["ReorderLevel"]
        temp_product["Discontinued"] = updated_product["Discontinued"]
        table_client.update_entity(
            mode=UpdateMode.MERGE, entity={**product, **temp_product}
        )
...
async def get_categories() -> Dict[str, Category]:
    ...
    Returns:
        A dictionary with category IDs as keys and Category objects as values.
    """
...
async def get_suppliers() -> Dict[str, Supplier]:
    ...
    Returns:
        A dictionary with supplier IDs as keys and Supplier objects as values.
    """
...
async def create_product(self,create_product: Product) -> str:
    ...
    # Get the next ID for the product
    next_id = await db_service.get_next_id(Settings.TABLE_NAME["PRODUCT"])
    
    # Create the new product entity as a dictionary
    new_product = {
        "etag": "",
        "PartitionKey": Settings.TABLE_NAME["PRODUCT"],
        "RowKey": str(next_id),
        "TimeStamp": datetime.now(),
        "ProductID": str(next_id),
        "ProductName": create_product.ProductName,
        "SupplierID": create_product.SupplierID,
        "CategoryID": create_product.CategoryID,
        "QuantityPerUnit": create_product.QuantityPerUnit,
        "UnitPrice": create_product.UnitPrice,
        "UnitsInStock": create_product.UnitsInStock,
        "UnitsOnOrder": create_product.UnitsOnOrder,
        "ReorderLevel": create_product.ReorderLevel,
        "Discontinued": create_product.Discontinued,
